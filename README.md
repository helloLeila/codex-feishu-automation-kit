...
设置页还可以保存默认作者、封面永久 MediaID、评论开关和自动同步防抖时间。第一次必须手动点击“创建微信草稿”；微信返回 `media_id` 后，默认编辑停止输入 30 秒会以 `body_only` 模式调用官方更新草稿接口，只更新正文和正文图片，不重新生成标题、摘要或封面。点击“进入审核”后自动同步会暂停，避免覆盖你在微信后台的手工修改。
