...
- 本机私密配置默认在 `~/.config/codex-feishu-automation-kit/tech-events-assistant.local.json`。它保存飞书 webhook、飞书签名密钥、Server 酱 SendKey，文件权限会设置为 `0600`。
- 工作区内的 `tech-events-assistant.local.json` 仍兼容旧工作流，并可覆盖用户级配置；它已经写进 `.gitignore`，不要提交。
...
用户级 `tech-events-assistant.local.json`

- 由 `npm run gba` 的配置步骤生成或更新。
- 默认路径是 `~/.config/codex-feishu-automation-kit/tech-events-assistant.local.json`。
- 不在仓库目录下，不提交到 Git。
- 保存真实推送密钥。
- 推荐新用户使用这个文件。
...
脚本读取时会从低到高合并配置：`$CODEX_HOME/codex-feishu-automation-kit/tech-events-assistant.local.json`、用户级 `~/.config/codex-feishu-automation-kit/tech-events-assistant.local.json`、当前工作区 `.env.local`、当前工作区 `tech-events-assistant.local.json`、`FEISHU_ENV_FILE` / `SERVERCHAN_ENV_FILE` 指定的额外 env 文件。最后，命令行进程里的 `FEISHU_WEBHOOK_URL`、`FEISHU_WEBHOOK_SECRET`、`SERVERCHAN_SENDKEY` 优先级最高。
...
- 直接回车：保留旧值。
- 输入 `clear`：清空当前值。
- 最后选择保存：写入用户级 `~/.config/codex-feishu-automation-kit/tech-events-assistant.local.json`。
- 最后选择不保存：原配置保持不变。
...
2. 生成 Markdown 活动清单。
3. 运行 `codex-feishu-push-gba-events <生成的Markdown文件路径>`。
4. 命令自动判断飞书和 Server 酱配置并推送到已配置通道。
5. 如果两个都配置，就两个都推送。
...
它只是启用通道，不包含真实 webhook。必须在用户级配置、工作区 `tech-events-assistant.local.json` 或 `.env.local` 里配置密钥。
...
先在本地运行 `npm run gba`，执行第 3 步测试真实连接；如果本地连接成功，再看自动化运行日志里有没有执行 `codex-feishu-push-gba-events`，以及全局命令是否已安装到自动化运行环境的 `PATH`。
