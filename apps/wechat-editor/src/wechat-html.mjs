...
export function inlineWechatHtml(source = '', theme = {}) {
  ...
  html = replaceOpeningTag(html, 'h1', `margin:0 0 ${paragraph + 4}px;color:${heading};font-size:${h1Size}px;line-height:1.28;font-weight:700;text-align:left`);
  html = replaceOpeningTag(html, 'h2', `margin:${section}px 0 13px;padding-left:11px;border-left:4px solid ${primary};color:${heading};font-size:${h2Size}px;line-height:1.35;font-weight:700;text-align:left`);
  html = replaceOpeningTag(html, 'h3', `margin:23px 0 9px;color:${heading};font-size:${h3Size}px;line-height:1.5;font-weight:700;text-align:left`);
  html = replaceOpeningTag(html, 'p', `margin:0 0 ${paragraph}px;color:${text};font-size:${bodySize}px;line-height:${lineHeight};text-align:left`);
  ...
  html = replaceOpeningTag(html, 'blockquote', `margin:20px 0;padding:13px 15px;border-left:3px solid ${quoteBorder};border-radius:0 4px 4px 0;background:${quoteBg};color:${quoteText};font-size:14px;line-height:1.75;text-align:left`);
  ...
  html = replaceOpeningTag(html, 'ul', `margin:0 0 ${paragraph}px;padding-left:1.4em;color:${text};font-size:${bodySize}px;line-height:${lineHeight};text-align:left`);
  html = replaceOpeningTag(html, 'ol', `margin:0 0 ${paragraph}px;padding-left:1.4em;color:${text};font-size:${bodySize}px;line-height:${lineHeight};text-align:left`);
  html = replaceOpeningTag(html, 'li', 'margin:0 0 6px;padding-left:2px;text-align:left');
  html = replaceOpeningTag(html, 'pre', `margin:18px 0;padding:14px;overflow-x:auto;border:1px solid ${border};border-radius:4px;background:${codeBg};color:${codeText};font:12px/1.7 SFMono-Regular,Menlo,monospace;white-space:pre-wrap;word-break:break-word;text-align:left`);
  ...
  html = replaceOpeningTag(html, 'table', `width:100%;border-collapse:collapse;border-spacing:0;color:${text};font-size:14px;line-height:1.6;text-align:left`);
  ...
  html = replaceOpeningTag(html, 'td', `padding:8px 10px;border:1px solid ${border};color:${text};vertical-align:top;text-align:left`);
  html = replaceOpeningTag(html, 'hr', `height:1px;margin:26px 0;border:0;border-top:1px solid ${border};text-align:left`);
  return html;
}
